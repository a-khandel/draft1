...
export function buildMessages(prompt: AgentPrompt): ModelMessage[] {
	...
	for (const part of Object.values(prompt)) {
		...
		if (!util) {
			console.error('No util found for prompt part type:', part.type)
			console.error('Available utils:', Object.keys(utils))
			throw new Error(`No util found for prompt part type: ${part.type}`)
		}
